...
void main() {
  group('NtsBurstClient.burst', () {
    test(
      'parallel mode: aggregates min-RTT and computes jitter floor',
      () async {
    ...
    test(
      'partial failures: aggregates over successes; failures preserved',
      () async {
        final client = testClient(
          nowFn: _fixedNow(_anchorMicros),
    ...
    test(
      'failures are sorted by issue index regardless of completion order',
      () async {
        ...
        expect(result.failures.map((f) => f.index).toList(), [0, 2, 4]);
      },
    );

    test(
      'whole-burst failure: returns hasResult=false with empty stats',
      () async {
        final client = testClient(
          nowFn: _fixedNow(_anchorMicros),
          rtts: const [-1, -1, -1],
    ...
    test(
      'rejects negative or oversized jitterWindow / sequentialSpacing',
      () async {
        final client = testClient(
          nowFn: _fixedNow(_anchorMicros),
          rtts: const [50000],
...
int Function() _fixedNow(int micros) =>
    () => micros;
