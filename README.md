...
| `validateBurstCount` | `int` | `8` | Tiered mode only: `getTime()` attempts per validate probe; the lowest-RTT sample is kept. Each attempt on a built-in NTS source may itself burst up to `ntsBurstCount` queries |
