...
5. **Back-compat fallback:** if no handle is registered, the worker falls back to the existing HTTPS HEAD probe. The probe preserves the pre-2.x connectivity-only contract: success on a 2xx response, otherwise `Result.retry()` on Android and `setTaskCompleted(success: false)` on iOS so the OS scheduler can back off and retry. Existing integrators that called `enableBackgroundSync` without ever calling `registerBackgroundCallback` keep their previous behaviour and do not break.
...
- **Pattern B: package-internal `@pragma('vm:entry-point')` callback (zero host-app changes).** Rejected. Tree-shaking in release-mode AOT can elide a callback that is referenced only from native code unless it is anchored from a host-app symbol. The community standard (`workmanager`, `flutter_local_notifications`) is host-registered for this exact reason. The one-line cost on the host app is acceptable for a security-sensitive package: the host-registered pattern lets us validate the *shape* of the callback (top-level/static) at registration time via `PluginUtilities.getCallbackHandle`, where a non-resolvable callback throws `ArgumentError` synchronously. The *retention* requirement (`@pragma('vm:entry-point')`) remains a build-time AOT concern that cannot be detected here — a missing pragma in release surfaces later, when the OS-spawned headless engine tries to look up the entrypoint by name in the AOT snapshot.
...
- Battery cost: a headless engine cold-start plus a multi-source NTP/NTS query is more expensive than an HTTPS HEAD. Mitigated by the `intervalMinutes` clamp (`>= 15`, matching WorkManager's hard periodic floor) and the OS schedulers' own throttling.
...
- The host app must add `@pragma('vm:entry-point')` to its callback. Two distinct failure surfaces: a callback of the wrong *shape* (closure or instance method) fails loudly at `registerBackgroundCallback` because `getCallbackHandle` returns null and we throw `ArgumentError`; a callback that is shaped correctly but missing the `@pragma` is *not* observable at registration — it only fails later, in release, when the OS-spawned headless engine tries to resolve the entrypoint in the tree-shaken AOT snapshot. In that latter case the worker's headless run errors out and falls through to no-op teardown; the next foreground launch still warm-restores from whatever anchor was last persisted.
...
|---|---|---|
| Host passed a closure or instance method | `getCallbackHandle` returns null at registration time | `registerBackgroundCallback` throws `ArgumentError` synchronously (debug or release) |
| Host forgot `@pragma('vm:entry-point')` | Headless engine cannot resolve the entrypoint in the AOT snapshot when the OS fires; `engine.run`/`executeDartCallback` errors out | Build-time only — add `@pragma('vm:entry-point')` to the top-level callback. Not detectable at runtime; verify by exercising a release build and observing whether the anchor advances |
