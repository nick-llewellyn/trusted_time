...
- API surface adds: `TimeSourceKind.nts`, `TimeSourceMetadata.authenticated` (defaulting to `false` for `ntp`/`https`/`custom`), `TrustedTimeConfig.ntsServers` (opt-in, empty by default). *(Historical note: this has since changed twice — NTS first shipped enabled with two anycast anchors, and `ntsServers` is now not a config field at all but a read-only view of a fixed curated inventory of 57 live-verified hosts, with `disableNts` as the only consumer control; see `trusted_time-cln`, `trusted_time-2tx`, `trusted_time-7pb`, and the [implementation status caveat](README.md#implementation-status-caveat).)*
...
Upstream `Sahad2701/trusted_time` shipped 2.0.0 on 2026-04-29 with NTS support implemented as **pure-Dart NTS-KE on top of `package:cryptography`** — the `package:basic_utils`-style approach this ADR rejected on RFC 5705 grounds. Upstream's own [CHANGELOG at v2.0.0](https://github.com/Sahad2701/trusted_time/blob/v2.0.0/CHANGELOG.md#200) (tag `v2.0.0`, commit [`71b371f`](https://github.com/Sahad2701/trusted_time/commit/71b371f9eb18ca2b419c47623e091cadc8bdb360)) labels the result a **"Cryptographic Preview"**, which is consistent with the constraint identified above: `dart:io.SecureSocket` does not expose the TLS keying-material exporter, and `package:cryptography` operates above the TLS layer, so a pure-Dart NTS-KE client running on the standard `dart:io` TLS stack cannot derive RFC 8915 §4.3-compliant C2S/S2C AEAD keys.
